...
let objectsVisible = true; // Новая переменная для отслеживания видимости объектов
...
const toggleObjectsButton = document.getElementById('toggle-objects'); // Новая кнопка
...
// Обработчик для новой кнопки "Показать/Скрыть объекты"
toggleObjectsButton.addEventListener('click', () => {
    objectsVisible = !objectsVisible; // Переключаем состояние видимости
    objects.forEach(obj => {
        obj.visible = objectsVisible; // Устанавливаем видимость каждого объекта
    });
    // Обновляем отображение полигонов (если объекты невидимы, их как бы "нет" для отображения статистики)
    if (objectsVisible) {
...
function addObjects() {
    ...
    for (let i = 0; i < objectCount; i++) {
        ...
        object.visible = objectsVisible; // Устанавливаем начальную видимость
        scene.add(object);
        objects.push(object);
        totalPolygons += singleObjectPolygons;
    }

    objCountDisplay.textContent = objectsVisible ? objectCount : 0; // Обновляем отображение
    polyCountDisplay.textContent = objectsVisible ? Math.round(totalPolygons).toLocaleString() : 0; // Обновляем отображение
}
...
function animate() {
    ...
    // Обновляем объекты только если они видимы
    if (objectsVisible) {
